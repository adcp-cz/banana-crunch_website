...
// 1. WAJIB LOGIN: Cek apakah user sudah masuk akun
...
// ===================================================
// FASE 1: MENAMPILKAN BARANG YANG DICENTANG DARI CART
// ===================================================
$items_to_buy = [];
$total_belanja = 0;

// Tangkap data array selected_items dari cart.php
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['selected_items'])) {
    $selected_cart_ids = $_POST['selected_items'];
    
    // Ambil detail data berdasarkan ID Cart yang dicentang
    ...
    // Amankan data ke dalam session agar tidak hilang saat menekan tombol submit form alamat
    ...
    // ===================================================
    // FASE 2: MEMPROSES PEMBELIAN AKHIR SAAT TOMBOL DIKLIK
    // ===================================================
    ...
    // Tangkap data tambahan dari form pengiriman
    ...
    // Mulai Database Transaction untuk keamanan integritas data relasional
    mysqli_begin_transaction($koneksi);

    try {
        // A. Masukkan baris data baru ke tabel orders
        // (Sesuaikan kolom tabel jika di database kamu memiliki kolom tambahan untuk alamat)
        $query_order = "INSERT INTO orders (user_id, total_price, status, created_at) 
                        VALUES ($user_id, $total_belanja, 'PROSES', NOW())";
        mysqli_query($koneksi, $query_order);
        $order_id = mysqli_insert_id($koneksi); // Ambil ID Order yang baru saja terbuat

        // B. Melakukan perulangan untuk memproses setiap item barang
        foreach ($items_to_buy as $item) {
            ...
            // 1. Validasi Stok Terkini di Gudang
            $res_cek = mysqli_query($koneksi, "SELECT stock FROM products WHERE id = $product_id");
            $data_stok = mysqli_fetch_assoc($res_cek);
            
            if ($data_stok['stock'] < $qty_beli) {
                throw new Exception("Stok untuk produk '" . $item['name'] . "' mendadak habis atau tidak mencukupi!");
            }

            // 2. Masukkan ke tabel order_details
            $query_detail = "INSERT INTO order_details (order_id, product_id, qty, price) 
                             VALUES ($order_id, $product_id, $qty_beli, $price)";
            mysqli_query($koneksi, $query_detail);

            // 3. LOGIKA POTONG STOK PRODUK SECARA OTOMATIS
            $query_update_stok = "UPDATE products SET stock = stock - $qty_beli WHERE id = $product_id";
            mysqli_query($koneksi, $query_update_stok);

            // 4. HAPUS ITEM TERCEDING HANYA YANG DIBELI DARI KERANJANG
            $query_hapus_cart = "DELETE FROM cart WHERE id = $cart_id AND user_id = $user_id";
            mysqli_query($koneksi, $query_hapus_cart);
        }

        // Kunci semua perubahan jika tidak ada error ditemui selama eksekusi
        mysqli_commit($koneksi);
        
        // Bersihkan variabel session checkout belanja
        unset($_SESSION['checkout_items']);
        unset($_SESSION['total_belanja']);

        echo "<script>
                alert('Hore! Pembelian Berhasil.\\nStok produk di database telah berkurang otomatis.');
                window.location.href = 'index.php';
              </script>";
        exit();

    } catch (Exception $e) {
        // Gagalkan semua transaksi jika di tengah jalan terdeteksi kesalahan stok
        mysqli_rollback($koneksi);
        echo "<script>
                alert('Gagal memproses pembayaran: " . $e->getMessage() . "');
                window.location.href = 'cart.php';
              </script>";
        exit();
    }
} else {
    // Jika user mencoba mengetik link checkout.php langsung tanpa memilih barang di cart
    header("Location: cart.php");
    exit();
}
?>
...
<head>
    ...
    <title>Checkout Pembayaran | PisangKraf</title>
...
<body>
    ...
    <nav class="navbar navbar-expand-lg fixed-top" style="padding: 15px 0; background: #fff; box-shadow: 0 4px 20px rgba(0,0,0,0.03);">
        <div class="container">
            <a class="navbar-brand fw-bold text-warning" href="index.php" style="color: var(--secondary) !important; font-size: 1.5rem;">PisangKraf</a>
            ...
            <div class="row g-4">
                    ...
                    <div class="checkout-card">
                        ...
                        <div class="mb-2">
                            ...
                            <select class="form-select" name="payment_method" required>
                                <option value="COD">Transfer Bank (Verifikasi Otomatis)</option>
                ...
                <div class="col-lg-5">
                    <div class="checkout-card">
                        ...
                        <button type="submit" class="btn btn-confirm">Konfirmasi & Selesaikan Pembelian</button>
                        <a href="chart.php" class="btn btn-light w-100 rounded-pill mt-2 text-muted border fw-medium py-2" style="font-size: 0.9rem;">Kembali ke Keranjang</a>
                    </div>
                </div>
